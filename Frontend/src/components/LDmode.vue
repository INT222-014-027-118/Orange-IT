<template>
    <div class="flex items-center cursor-pointer select-none" :class="[this.$store.getters.isAdmin ? 'mt-2 py-3 w-full shadow-sm' : 'flex-wrap']" @click="changeSetChangeMode()">
        <div class="justify-center" :class="[this.$store.getters.isAdmin ? 'flex sm:ml-3 md:ml-6 w-full sm:w-auto' : 'w-14 md:w-16 hidden sm:inline-flex']">
            <div
            ...
            >
                <div
                    ...
                    :class="[this.$store.getters.isAdmin ? 'w-5 h-5 p-0.5' : ''] + [this.$store.getters.changeMode == true ? '-translate-x-0 bg-primary ring-primary' : 'translate-x-4 bg-blue-500 ring-blue-500']"
                >
                    <span class="material-icons-round text-sm">{{ this.$store.getters.changeMode == true ? "wb_sunny" : "dark_mode" }}</span>
                </div>
            </div>
        </div>
        <span class="font-semibold capitalize" :class="[this.$store.getters.isAdmin ? 'text-sm md:text-base text-left lg:text-lg hidden sm:inline-block ml-3' : 'py-2 ml-3 sm:py-0 sm:ml-1']">
